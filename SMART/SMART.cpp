...
/***** Global variable *****/
/***************************/
const double goalX = 30.01;
const double goalY = 30.01;
...
/******* SMART Class *******/
/***************************/
SMART::SMART(int _trialIndex, double _obstacleSpeed, double _robotSpeed, int scene_index, int _dynObsNum, vector<vector<double>> dynObsPosition): 
treeRoot(new node(goalX, goalY)), TRH(0.8), TOH(0.4), robotSpeed(_robotSpeed), obstacleSpeed(_obstacleSpeed),
obstacleRadius(1.0), reachThreshold(0.5), totalReplanTime(0.0), totalTravelTime(0.0), averageReplanTime(0.0), totalReplanNum(0), collisionCheckingResolution(0.1),
dynObsNum(_dynObsNum), iterationIndex(0), mainTreeID(1), newTreeID(1), LSR_size_current(3), LSR_size_max(2* max(mapROW, mapCOL) - 1),
initDone(false), replanTrigger(false), robotInsideOHZ(false),
...
obstacle_heading(uniform_real_distribution<double>(0.0, 2.0 * M_PI))
{
	...
	for (int row = 0; row < mapROW; row++)
	{
		for (int col = 0; col < mapCOL; col++)
		{
			...
			tiling[row][col].x = x;
			tiling[row][col].y = y;
			tiling[row][col].distToGoal = sqrt((x - goalX) * (x - goalX) + (y - goalY) * (y - goalY));
			tiling[row][col].distToRobot = sqrt((x - robotX) * (x - robotX) + (y - robotY) * (y - robotY));
			tiling[row][col].utility = 0;
			tiling[row][col].status = unknown;
		}
	}
	searchCenter.i = -1;
	searchCenter.j = -1;
...
SMART::~SMART(void)
{
	...
	// Reset the robot pose with the initial value for next trial
	robotX = 2;
	robotY = 2; 
}
...
{
	double spaceXmax = (double) mapCOL * cellSize;
	double spaceYmax = (double) mapROW * cellSize;
...
{
	int staticObsMap[mapROW][mapCOL];
	ifstream file;
	file.open("staticObsMap.txt");
	if (!file.is_open()) 
	{
		printf("Manually create the static obstacle map...\n");
		...
		// Create virtual border
		for (int j = 0; j < mapCOL; j++)
		{
			staticObsMap[0][j] = 1;
			staticObsMap[mapROW-1][j] = 1;
		}
		for (int i = 0; i < mapROW; i++)
		{
			staticObsMap[i][0] = 1;
			staticObsMap[i][mapCOL-1] = 1;
		}
	}
	else
	{
		printf("Load static obstacle map successfully...\n");
		for (int i = 0; i < mapROW; i++) 
		{
			for (int j = 0; j < mapCOL; j++) 
			{
				file >> staticObsMap[i][j];
			}
		}
		file.close();
	}
	for (int i = 0; i < mapROW; i++)
	{
		for (int j = 0; j < mapCOL; j++)
		{
			if (staticObsMap[i][j] == 0)
			{
				staticObsFreeCell.push_back({i, j});
			}
			else
			{
				tiling[i][j].status = inStaticObs;
			}
		}
	}
	/*for (int i = 0; i < mapROW; i++)
...
{
	if (row >= 0 && row < mapROW && col >= 0 && col < mapCOL) return true;
	return false;
}
